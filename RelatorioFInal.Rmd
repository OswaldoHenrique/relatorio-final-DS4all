...
Os dados abaixos foram fornecidos pelos professores responsáveis da disciplina. Os dados foram gerados no mês de maio de 2018, e compilam informações entre os anos de 2010 e 2017. Os arquivos estão no formato JSON.
...
Podemos notar também que temos 5 grandes áreas no programa de Ciências Médicas, 8 no de Saúde Coletiva Profissional e 9 no de Saúde Coletiva Profissional. Em relação as áreas, temos 22, 24 e 24, respectivamente. Já para sub-áreas, temos 75, 64 e 49, respectivamente. Finalmente, para especialidades, temos 40, 21 e 21, respectivamente.
...
Entretanto, cada tipo de publicação possui dados diferentes, como é o caso dos periódicos e dos eventos, por exemplo. Os dados dos periódicos são id, natureza, título, ano, volume, issn, páginas, doi, autores, autores endogeno.
...
Já os dados dos eventos são identificador, natureza, título, nome do evento, ano do trabalho, país do evento, cidade do envento, doi, classificação, páginas, autores e autores endogeno.
```{r}
...
```
E por ultimo, para o programa de Saude coletiva Profissional temos:
...
Alem disso, para uma futura junção entre as bases do Lattes com as outras foi criada uma tabela de cada uma das bases de dados contendo o nome e o id lattes dos professores
...
A linha de pesquisa mais frequente em Ciências Médicas é a respeito da Manifestação digestiva da doença de Chagas, em Saúde Coletiva pe a respeito da segurança Alimentar e Nutricional de populações: diagnóstico e estratégias públicas, por ultimo, em Profissional Saúde COletiva a linha de pesquisa mais frequente é o Processo de trabalho, práticas de atenção, gestão e formação em saúde.
...
x = "Especialidade", y = "Qtd de professores")
```
Com a análise dos graficos é possível perceber que é alto o numero dos que não tem especialidade
...
ggplot(total, aes(x = "", y=valores, fill = tipo)) + geom_col() + coord_polar(theta = "y")
```
Analisando o numero de professores relacionados ao numero de alunos é possível perceber que tanto em Saúde Coletiva, Ciências Médicas e Profissional Saúde Coletiva possuem muito mais alunos que professores e, comparando os 3, os valores são bem próximos
...
scale_y_continuous(breaks = pretty(publication_quant$quantidade, n =20)) +
labs(x = "ano", y= "publicações")+
ggtitle("Quantidade de periodicos por ano Saude Coletiva Profissional")
```

Desde o ano de 2010 o nÚmero de periódicos vem aumentando, embora o valor do ano de 2017 estja bem menor, isso se deve ao fato de que o ano ainda não acabou, não podendo ser levado em consideração para as análises. O fato do crescimento do número de periódicos por ano deve-se ao incentivo por publicações realizada pela UnB. Podemos esperar que estes numeros continuem crescendo pois foi lançado o primeiro edital de auxílio à publicação de pesquisa científica, tecnológica e de inovação de docentes do quadro da Universidade de Brasília pelo DPI. Que visa estimular e viabilizar a divulgação de artigos em anais de eventos ou em periódicos, nacionais e internacionais.
...
geom_histogram(stat = "count", fill = "#ff9999") +
labs(x = "classificação do evento", y= "Quantidade de publicações") +
ggtitle("Quantidade de publicaçoes por tipo de evento Saude Coletiva Profissional")
```

Pela análise da quantidade de publicações por tipo, percebe-se que há um maior nível de publicações internacionais seguida pelas publicações nacionais, sendo Ciências Médicas a que possui os maiores números.
...
No caso dos professores do programa Profissional Saúde Coletiva temos:
```{r}
...
O grupo de pesquisa Medicina Tropical é o que possui o maior numero de publicações tanto em Saúde Coletiva quanto em Profissional Saúde Coletiva. Entretanto em Ciências Médicas o grupo Arte e Tecnociência possui o total de 511 parcerias, superando Medicina Tropical nos outros 2 programas.

É interessante reparar que os grupos de pesquisa de Saúde Coletiva e Profissional de Saúde coletiva tem os mesmos valores, podemos concluir que eles são os mesmos para ambos os programas e serão tratados como 1 só. Além disso, em Ciências Médicas há o grupo de Medicina Tropical, entretanto há outros grupos específicos deste programa que possuem valores maiores que os grupos de pesquisa que também são incluidos no programa de Saúde COletiva.


Alem da quantidade de publicações, também podemos analisar as parcerias dos professores
...
arrange(desc(N))
```
Novamente o grupo de pesquisa Medicina Tropical é o que possui o maior numero de parcerias tanto em Saúde Coletiva quanto em Profissional Saúde Coletiva. Entretanto em Ciências Médicas o grupo Laboratório de Desenvolvimento de Inovações Terapêuticas possui o total de 13 parcerias, superando Medicina Tropical nos outros 2 programas.
